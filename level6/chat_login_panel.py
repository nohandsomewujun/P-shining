...
class LoginPanel:
    ...
    def show_login_panel(self):
        ...
        numIdx = 50  # gif的帧数
        # 循环遍历动图的帧
        frames = [ImageTk.PhotoImage(file='login.gif',format='gif -index %i' % (i)) for i in range(numIdx)]
         #img_open = Image.open('F:/the_end_of_2nd/multi_python/level6/bg.gif')
        # img_gif = ImageTk.PhotoImage(img_open)
        #im = Image.open('login.gif')
        #frames = ImageSequence.Iterator(im)
        #numIdx = frames.__sizeof__()
        # 创建存放gif的标签
        imgLabel = Label(self.login_frame, height=400, width=500)
        # 设置标签的位置
        imgLabel.place(x=-252, y=-200, relx=0.5, rely=0.5, relwidth=1, relheigh=0.5)
        #imgLabel.configure(image=img_gif)
        # 设置文本标签和位置
